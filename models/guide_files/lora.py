...
class ScriptArguments:
    ...
    warmup_steps: int = field(
        default=100, metadata={"help": "# of steps to do a warmup for"}
    )
    ...
    save_steps: int = field(
        default=200, metadata={"help": "Save checkpoint every X updates steps."}
    )
    logging_steps: int = field(
        default=5, metadata={"help": "Log every X updates steps."}
    )
    ...
    for sample in iter(ds):
        ...
        if input_text is None or input_text == "":
            formatted_prompt = (
                ...
                f"{str(out_text)}" f"<|eot_id|><|end_of_text|>",
            )
        else:
            formatted_prompt = (
                f"<|begin_of_text|><|start_header_id|>user<|end_header_id|>\n\n"
